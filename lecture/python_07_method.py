...
# 4. return: 함수 종료 의미
# 5. return 반환값: 함수종료+호출문으로 변환값 전달(tuple)
# 6. 함수 종료: 1. return, 2. 들여쓰기 끝
# 7. parameter와 return을 생략가능
#   (입력과 변환이 없는 함수도 존재)
...
def sub_two_value(x: int, y: int):      # type 힌트   ~~ : int
    n = x-y
    return n
a,b = 15, 20
...
# 4. Default Parameter
# - 함수 호출시 parameter를 전달 받지 못한 경우 기본값 사용, 오른쪽부터 채워넣기
# def test(a, b, c=3):          # O
